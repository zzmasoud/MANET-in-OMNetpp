...
void Aodv::initialize(int stage)
{
    if (stage == INITSTAGE_LOCAL) {
        ...
        zzServices = par("zzServices");
        zzNeedServices = par("zzNeedServices");
        std::cout<<"AODV SERVICES: ";
        std::cout<<zzServices;
        std::cout<<"AODV NEED SERVICES: ";
        std::cout<<zzNeedServices;
        std::cout<<"\n\n";
...
const Ptr<Rreq> Aodv::createRREQ(const L3Address& destAddr)
{
    ...
    rreqPacket->setZZRequestedService_req(this->zzServices);
...
const Ptr<Rrep> Aodv::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    ...
    rrep->setZRZequestedService_rep(this->zzServices);
...
void Aodv::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    ...
    if (rrep->getZZRequestedService_rep() != this->zzServices) {
        EV_INFO << "The sender requested service: " << rrep->getZZRequestedService_rep() << " is not available in this node." << endl;
        return;
    }
...
const Ptr<Rerr> Aodv::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
    ...
    rerr->setZZRequestedService_rer(this->zzServices);
...
void Aodv::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
    ...
    if (rerr->getZZRequestedService_rer() != this->zzServices) {
        EV_INFO << "The sender requested service: " << rerr->getZZRequestedService_rer() << " is not available in this node." << endl;
        return;
    }
