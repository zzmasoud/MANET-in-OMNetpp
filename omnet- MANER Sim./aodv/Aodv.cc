...
void Aodv::initialize(int stage)
{
    if (stage == INITSTAGE_LOCAL) {
        ...
        zzAvailableServices = "1,2,3";
        zzRequestedServices = "4";

//        zzAvailableServices = par("zzAvailableServices");
//        zzRequestedServices = par("zzRequestedServices");

//        zzAvailableServices = getModuleFromPar<std::string>(par("zzAvailableServices"), this);
//        zzRequestedServices = getModuleFromPar<std::string>(par("zzRequestedServices"), this);



        std::cout<<"AODV SERVICES: ";
        std::cout<<zzAvailableServices;
        std::cout<<"AODV NEED SERVICES: ";
        std::cout<<zzRequestedServices;
        std::cout<<"\n\n";
...
const Ptr<Rreq> Aodv::createRREQ(const L3Address& destAddr)
{
    ...
    rreqPacket->setZZRequestedServices_req(this->zzRequestedServices);
...
const Ptr<Rrep> Aodv::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    ...
    rrep->setZZRequestedServices_rep(this->zzRequestedServices);
...
void Aodv::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    ...
    std::string requests = rrep->getZZRequestedServices_rep();

    size_t pos = 0;
    while ((pos = requests.find(",")) != std::string::npos) {
        list.push_back(requests.substr(0, pos));
        requests.erase(0, pos + 1);
    }


    bool isAvailable = false;
    for(const std::string &service : list) {
        if (this->zzAvailableServices.find(service) != std::string::npos ) {
            isAvailable = true;
        }
    }

    if (!isAvailable) {
        EV_INFO << "The sender requested services: " << rrep->getZZRequestedServices_rep() << " is not available in this node." << endl;
        return;
    }
...
void Aodv::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    ...
    std::vector<std::string> list;
    std::string requests = rreq->getZZRequestedServices_req();
     ...
     EV_INFO << "LIST OF SERVICES: " << list[0];
        bool isAvailable = false;
        for(const std::string &service : list) {
            if (this->zzAvailableServices.find(service) != std::string::npos ) {
                isAvailable = true;
        ...
        if (!isAvailable) {
            EV_INFO << "The sender requested services: " << rreq->getZZRequestedServices_req() << " is not available in this node." << endl;
            return;
...
const Ptr<Rerr> Aodv::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
    ...
    rerr->setZZRequestedServices_rer(this->zzRequestedServices);
...
void Aodv::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
    ...
    std::vector<std::string> list;
    std::string requests = rerr->getZZRequestedServices_rer();
     ...
     while ((pos = requests.find(",")) != std::string::npos) {
        ...
        if (!isAvailable) {
            EV_INFO << "The sender requested services: " << rerr->getZZRequestedServices_rer() << " is not available in this node." << endl;
            return;
        }
